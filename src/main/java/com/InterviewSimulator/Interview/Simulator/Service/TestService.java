...
public class TestService {
    ...
    public TestResponseDto getTodayTest() {
        Test test = testRepository
                ...
                .orElseThrow(() -> new RuntimeException("No test available"));
    ...
    public TestResultDto  getTodayTestResult(TestSubmitRequestDto dto) {
        ...
        for (AnswerDto answer : dto.getAnswer()) {
            ...
            // CODING
                ...
                for (Test_cases tc : testcases) {
                    ...
                    String output = judge0Service.runCode(
                            code,
                            cleanInput,
                            answer.getLanguage()
                    );
                    ...
                    if (output != null && output.contains("\"stdout\":\"")) {
                        int start = output.indexOf("\"stdout\":\"") + 10;
                        int end = output.indexOf("\"", start);

                        if (end > start) {
                            actualOutput = output.substring(start, end);
                        }
                    }
                    ...
                    System.out.println("RAW ACTUAL = [" + actualOutput + "]");
                    ...
                    System.out.println("FIXED ACTUAL = [" + actualOutput + "]");
        ...
        attempt.setTheory_score(theoryscore);
        System.out.println("CODING SCORE = " + codingscore);
        System.out.println("THEORY SCORE = " + theoryscore);
    ...
    public ResponseEntity<?> RunCode(long questionId , String code, String language){
         ...
         for (Test_cases tc : testcases) {
             ...
             String output = judge0Service.runCode(code, cleanInput, language);
             String actualOutput = output;
             ...
             else if (actualOutput.trim().equals(tc.getExpectedoutput().trim())) {
